...
GLuint Texture::loadTexture(const std::string &fName) {
    ...
    GLuint tex = 0;
    if (data != nullptr) {
        std::cout << "Texture loaded: " << fName << " (" << width << "x" << height << ")" << std::endl;
        glGenTextures(1, &tex);
...
GLuint Texture::loadCubeMap(const std::string &baseName, const std::string &extension) {
    ...
    if (data != nullptr) {
        std::cout << "Cube map face loaded: " << texName << " (" << w << "x" << h << ")" << std::endl;
    } else {
        std::cerr << "Failed to load cube map face: " << texName << std::endl;
    }
...
GLuint Texture::loadHdrCubeMap(const std::string &baseName) {
    ...
    if (data != nullptr) {
        std::cout << "HDR cube map face loaded: " << texName << " (" << w << "x" << h << ")" << std::endl;
    } else {
        std::cerr << "Failed to load HDR cube map face: " << texName << std::endl;
    }
